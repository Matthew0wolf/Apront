...
def create_invite():
    ...
    role = data.get('role')
    company_id = g.current_user.company_id  # Usa a empresa do usuário logado
    ...
    # Loga o uso
    log_usage(company_id, g.current_user.id, 'invite_user', 'user', invite.id, {'email': email, 'role': role})
